...
[dnl
  ...
  AC_ARG_ENABLE([targets],
    [AS_HELP_STRING([--enable-targets],
       ...
       host, vanilla, foonvdla, x86, x86_64, sparc, powerpc, alpha, aarch64, arm,
  ...
  case "$enableval" in
    all) TARGETS_TO_BUILD="Vanilla FooNvdla X86 Sparc PowerPC Alpha AArch64 ARM NvDla Mips Hexagon CellSPU XCore MSP430 SystemZ Blackfin CBackend CppBackend MBlaze PTX"
        ;;
    *)for a_target in `echo $enableval|sed -e 's/,/ /g' ` ; do
        case "$a_target" in
    foonvdla)
        TARGETS_TO_BUILD="FooNvdla $TARGETS_TO_BUILD"
        AC_DEFINE(ENABLE_FOONVDLA_TARGET, 1, [define FooNvdla target])
        ;;
    vanilla)
  ...
  AM_CONDITIONAL([ENABLE_FOONVDLA_TARGET], [ test "${TARGETS_TO_BUILD/FooNvdla}" != "${TARGETS_TO_BUILD}" ])
